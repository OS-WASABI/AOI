...
std::optional<Alert> Alert::from_json(web::json::value &alert_json) {
    ...
    try {
        ...
        // Required
        if (!alert_json.has_field("sender")) {
            if (!alert.validate_sender(""))
                return std::nullopt;
        } else {
        ...
        // Required
        if (!alert_json.has_field("sent_time")) {
            if (!alert.validate_sent_time(0))
                return std::nullopt;
        } else {
        ...
        //Required
        if (!alert_json.has_field("status")) {
            if (!alert.validate_status(""))
                return std::nullopt;
        } else {
        ...
        // Required
        if (!alert_json.has_field("msg_type")) {
            if (!alert.validate_msg_type(""))
                return std::nullopt;
        } else {
        ...
        // Required
        if (!alert_json.has_field("scope")) {
            if (!alert.validate_scope(""))
                return std::nullopt;
        } else {
...
bool Alert::validate_sent_time(const time_t &sent_time) {
    logger__.Log(aoi_rest::LogLevel::DEBUG, "Validating sent time: " + std::to_string(sent_time), "Alert.cpp", "validate_sender");
